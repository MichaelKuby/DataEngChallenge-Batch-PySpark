...
def filter_for_valid_countries(df, spark):
    ...
    df_valid_countries = df.join(broadcast(valid_countries_df), df['Country'] == valid_countries_df['ValidCountry'], 'inner')
    df_valid_countries = df_valid_countries.withColumn('Country', initcap('ValidCountry')).drop('ValidCountry')
